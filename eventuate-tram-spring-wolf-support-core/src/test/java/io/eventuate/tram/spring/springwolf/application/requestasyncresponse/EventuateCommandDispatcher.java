...
import io.eventuate.tram.spring.springwolf.application.requestasyncresponse.replies.FailureReply;
import io.eventuate.tram.spring.springwolf.application.requestasyncresponse.replies.SuccessReply;
...
public class EventuateCommandDispatcher implements SmartLifecycle {
  ...
  private static <T extends Command> Object invokeMethod(CommandHandlerInfo handler, CommandMessage<T> cm) {
    Object reply;
    try {
      reply = handler.method().invoke(handler.bean(), cm);
    } catch (Exception e) {
